...
def mk_out_filenames(opts, n_vars, t):
    prefix = "%s/sr_n=%.4d_pk2=%.2f_pg=%.2f_t=%d" % (opts.out_dir, n_vars, opts.p_k_2, opts.p_geo, t)
...
def gen_iclause_pair(opts, max_len):
    while True:
        ...
        if len(iclauses) <= max_len - opts.max_n - 3:
...
if __name__ == "__main__":
    ...
    for pair in range(opts.n_pairs):
        ...
        n_vars, iclauses, iclause_unsat, iclause_sat = gen_iclause_pair(opts, opts.max_len)

        with open(opts.filename, "a") as f:
            iclauses.append(iclause_unsat)
            clause_strings = [" ".join(map(str, clause)) for clause in iclauses]
            formatted_clauses = " ^ ".join(clause_strings)
            f.write(f"{formatted_clauses} <sep> sat\n")

            iclauses[-1] = iclause_sat
            clause_strings = [" ".join(map(str, clause)) for clause in iclauses]
            formatted_clauses = " ^ ".join(clause_strings)
            f.write(f"{formatted_clauses} <sep> unsat\n")
